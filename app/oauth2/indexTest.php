...
class CleverInstantLoginExample extends PHPUnit_Framework_TestCase {
  ...
  public function testSetOptionsRequiresCriticalConfiguration() {
    try {
      putenv("41f56eff7fe5d53a8096");
      putenv("692914f2e8108d31bfe650414ea79c82f288a36b");
      $options = set_options();
  ...
  public function testRequestFromClever() {
    ...
    $response = request_from_clever("http://localhost:8082", array(), $mock_request_options);
  ...
  public function testErrorHandlingFromCurl() {
    $mock_request_options = prepare_options_for_clever();
    try {
      $response = request_from_clever("httpl://explorer:8082", array(), $mock_request_options);
      $this->fail("Expected exception not thrown");
    } catch(Exception $e) {
...
function prepare_options_for_clever() {
  $options = array(
    'client_id' => 'abc',
    'client_secret' => 'def',
    'client_redirect_url' => 'http://localhost:8082/oauth2',
    'clever_redirect_base' => 'http://localhost:8082',
    'clever_oauth_tokens_url' => 'http://localhost:8082/oauth2/tokens',
    'clever_api_me_url' => 'http://localhost:8082/me',
    "clever_oauth_authorize_url" => 'http://localhost:8082/oauth/authorize',
  );
  return $options;
}
